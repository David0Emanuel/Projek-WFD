...
<div class="grid grid-cols-1 gap-6 sm:grid-cols-1 lg:grid-cols-2">
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <p class="mt-2 text-3xl font-black text-blue-600">{{ $totalKamar > 0 ? round(($terisiCount / $totalKamar) * 100) : 0 }}%</p>
        <p class="mt-1 text-sm text-gray-500">{{ $terisiCount }} dari {{ $totalKamar }} Kamar Terisi</p>
    </div>

    <!-- Card Menunggu Check-in -->
    <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        <p class="text-xs font-bold uppercase tracking-wider text-gray-500">Menunggu Check-in</p>
        <p class="mt-2 text-3xl font-black text-amber-500">{{ $waitingCheckinCount }}</p>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <p class="mt-2 text-3xl font-black text-red-500">{{ $pendingTiketCount }}</p>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <p class="mt-2 text-3xl font-black text-green-600">{{ $todaySurveyCount }}</p>
