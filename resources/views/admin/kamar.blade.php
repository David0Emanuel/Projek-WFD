...
<div id="maintenanceModal" class="fixed inset-0 z-[100] hidden items-center justify-center bg-gray-900/60 p-4 opacity-0 backdrop-blur-sm transition-opacity duration-300">
    <div class="w-full max-w-md scale-95 transform rounded-3xl bg-white p-6 shadow-2xl transition-transform duration-300 text-center">
        ...
        <p class="mt-2 text-sm text-gray-600">Apakah Anda yakin ingin mengubah status Kamar <span id="teks_nomor_kamar_maintenance"></span>?</p>
        <div class="mt-6 flex gap-3">
            ...
            <button type="button" onclick="closeMaintenanceModal()" class="w-full rounded-xl bg-amber-500 px-4 py-3 text-sm font-bold text-white transition-colors hover:bg-amber-600">Ya</button>
        </div>
    </div>
</div>

<script>
    // --- MANAJEMEN OPERASI MODAL ---
    ...
    document.getElementById('formMeteran').addEventListener('submit', function(e) {
        ...
        .then(async response => {
            ...
            if (response.ok) {
                ...
                // Ekstrak pesan validasi detail per field jika ada
                if (data.errors) {
        ...
        .catch(error => {
            ...
            // Menampilkan pesan error validasi/database asli ke User
