        ...
        <div class="flex flex-col justify-between rounded-2xl border border-gray-200 bg-white p-5 shadow-sm transition-all hover:shadow-md">
            ...
            <div class="mt-4 space-y-3">
                ...
                @else
                    ...
                    <button type="button" onclick="openMeteranModal({{ $kamar->id }}, '{{ $kamar->nomor }}')" class="w-full rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-bold text-white transition-colors hover:bg-blue-700 cursor-pointer">
                        Input Meteran Listrik
                    </button>
                @endif
            </div>
...
<div id="meteranModal" class="fixed inset-0 z-[100] hidden items-center justify-center bg-gray-900/60 p-4 opacity-0 backdrop-blur-sm transition-opacity duration-300">
    <div class="w-full max-w-md scale-95 transform rounded-3xl bg-white p-6 shadow-2xl transition-transform duration-300">
        ...
        <form id="formMeteran" action="{{ route('admin.kamar.meteran') }}" method="POST" enctype="multipart/form-data" class="mt-5 space-y-4">
            @csrf
            <input type="hidden" name="kamar_id" id="input_kamar_id_meteran">

            <div>
                <label for="total_tagihan" class="mb-1.5 block text-xs font-bold uppercase tracking-wider text-gray-600">Total Tagihan (Rp)</label>
                <input type="number" id="total_tagihan" name="total_tagihan" required placeholder="Contoh: 150000"
            ...
            <div>
                ...
                <input type="file" id="foto_meteran" name="foto_meteran" required accept="image/*"
...
>>>>>>> 5b06971a36c99faf7c2cade687c32ee6748609d7
...
<script>
    ...
    function openMeteranModal(id, nomor) {
        document.getElementById('input_kamar_id_meteran').value = id;
        document.getElementById('teks_nomor_kamar_meteran').innerText = nomor;
        
        // Opsional: Jika kamu sudah membuat route-nya, kamu bisa ubah action form secara dinamis di sini
        // document.getElementById('formMeteran').action = "/admin/kamar/meteran/" + id;
    ...
    function closeMeteranModal() {
        ...
        setTimeout(() => {
            ...
            document.getElementById('formMeteran').reset(); // reset form saat ditutup
        }, 300);
    }
    ...
    function openMaintenanceModal(id, nomor) {
        ...
        // Opsional: Ubah action form secara dinamis
        // document.getElementById('formMaintenance').action = "/admin/kamar/maintenance/" + id;
